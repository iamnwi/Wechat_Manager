...
def login(self, enableCmdQR=False, picDir=None, qrCallback=None,
        loginCallback=None, exitCallback=None, **extra_dict):
    ...
    while self.isLogging:
        ...
        if uuid:
            ...
            if hasattr(qrCallback, '__call__'):
                if 'request' in extra_dict.keys():
                    req = extra_dict['request'];
                    qrCallback(uuid=self.uuid, qrcode=qrStorage.getvalue(), request=req)
                else:
                    qrCallback(uuid=self.uuid, qrcode=qrStorage.getvalue())
            logger.info('Please scan the QR code to log in.')
        isLoggedIn = False
        while not isLoggedIn:
            status = self.check_login(uuid)
            #if hasattr(qrCallback, '__call__'):
...
# def get_QR(self, uuid=None, enableCmdQR=False, picDir=None, qrCallback=None, **extra_dict):
def get_QR(self, uuid=None):
    ...
    # if hasattr(qrCallback, '__call__'):
    #     qrCallback(uuid=uuid, status='0', qrcode=qrStorage.getvalue(), extra_dict)
    # else:
    #     if enableCmdQR:
